...
from emishows.icalendar.errors import DifferentTimezonesError
from emishows.icalendar.models import (
    Calendar,
    Event,
    Frequency,
    Recurrence,
...
class ICalendarParser:
    ...
    def datetime_to_ical(self, dt: datetime) -> icalendar.vDatetime:
        ...
        timezone = tzid_from_dt(dt)
        if timezone == "Etc/UTC":
            timezone = "UTC"

        fdt = dt.strftime("%Y%m%dT%H%M%S")
        icaldt = icalendar.vDatetime.from_ical(fdt, timezone)
    ...
    def datetimes_to_ical(self, dts: list[datetime]) -> vDDDLists:
        ...
        timezones = set([tzid_from_dt(dt) for dt in dts])

        if len(timezones) > 1:
            raise DifferentTimezonesError()
        ...
        icaldts = vDDDLists.from_ical(icaldts, timezone)
    ...
    def ical_to_datetimes(self, vdts: vDDDLists) -> list[datetime]:
        ...
        timezone = vdts.params.get("TZID")

        icaldts = vdts.to_ical().decode("utf-8")
        icaldts = icaldts.split(",")
        icaldts = [
            icalendar.vDatetime(icalendar.vDatetime.from_ical(icaldt, timezone))
            for icaldt in icaldts
        ]
        return [self.ical_to_datetime(icaldt) for icaldt in icaldts]

    def frequency_to_ical(self, frequency: Frequency) -> icalendar.vFrequency:
    ...
    def ical_to_frequency(self, vfrequency: icalendar.vFrequency) -> Frequency:
    ...
    def weekday_to_ical(self, weekday: Weekday) -> icalendar.vWeekday:
    ...
    def ical_to_weekday(self, vweekday: icalendar.vWeekday) -> Weekday:
    ...
    def ical_to_weekday_rules(
        ...
        for weekday in vweekdays:
            ...
            rule = WeekdayRule(day=day, occurrence=occurrence)
            results.append(rule)

        return results

    def recurrence_rule_to_ical(self, rule: RecurrenceRule) -> icalendar.vRecur:
    ...
    def ical_to_recurrence_rule(self, vrecur: icalendar.vRecur) -> RecurrenceRule:
    ...
    def event_to_ical(self, event: Event) -> icalendar.Event:
        ...
        if recurrence is not None:
            ...
            if recurrence.include is not None:
                rdate = [
                    dt.replace(tzinfo=ZoneInfo(event.timezone))
                    for dt in recurrence.include
                ]
                ical.add("rdate", self.datetimes_to_ical(rdate))
            if recurrence.exclude is not None:
                exdate = [
                    dt.replace(tzinfo=ZoneInfo(event.timezone))
                    for dt in recurrence.exclude
                ]
                ical.add("exdate", self.datetimes_to_ical(exdate))

        return ical

    def ical_to_event(self, event: icalendar.Event) -> Event:
        ...
        timezone = tzid_from_dt(start.dt)
        ...
        return Event(
            id=id, start=start, end=end, timezone=timezone, recurrence=recurrence
        )

    def calendar_to_ical(self, calendar: Calendar) -> icalendar.Calendar:
    ...
    def ical_to_calendar(self, calendar: icalendar.Calendar) -> Calendar:
